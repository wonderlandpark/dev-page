...
function updateSyntax() {
    ...
    if (desc && expected && actual && client && system && steps) {
        if (desc.includes(' | ') || expected.includes(' | ') || actual.includes(' | ') || client.includes(' | ') || system.includes(' | ') || steps.includes(' | ')) bugtext = 'ERR_CODE : INCLUDES_INVAILD_TEXT\n잘못된 텍스트가 포함되었습니다.\n 잘못된 텍스트 : " | "'
        else bugtext = '!bug ' + desc + ' | ' + steps + ' | ' + expected + ' | ' + actual + ' | ' + client + ' | ' + system;
    }
    $('#syntax').text(bugtext);
